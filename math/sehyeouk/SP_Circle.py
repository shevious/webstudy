...
numB = 21  # Number of boundary points (including endpoint) - n각형 >> 2n +1
...
'''
    ...
    if i%2 == 1 :
        XB[i] = ((numPan-1)/numPan)*np.cos(theta)
        YB[i] = ((numPan-1)/numPan)*np.sin(theta)
'''
...
for i in range(numPan):  # Loop over all panels
    if i%2 == 1:
         XB[i] = ((numPan-5)/numPan)*XB[i]
         YB[i] = ((numPan-5)/numPan)*YB[i]
...
if (flagPlot[2] == 1):
    ...
    plt.cla()  # Get ready for plotting
    plt.plot(analyticTheta * (180 / np.pi), analyticCP, 'b-',
             label='Analytical')  # Plot analytical pressure coefficient
